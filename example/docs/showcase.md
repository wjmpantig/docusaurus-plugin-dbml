...
One schema exercising everything at once: multiple schemas, table groups, sticky
notes, enums, indexes, checks, composite keys, and every relationship kind.

This is the same sample the renderer uses as its own development fixture, so it
doubles as a regression surface — if a DBML feature stops rendering, it shows up
here first.

It is set to `height=800`. Try hovering a column to highlight its relationships,
dragging a table, and toggling this site between light and dark.
